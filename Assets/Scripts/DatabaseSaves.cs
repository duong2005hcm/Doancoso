...
public class DatabaseSaves : MonoBehaviour
{
    ...
    public void UpdateMetersTraveled(float metersTraveled)
    {
        ...
        databaseReference.Child("Users").Child(currentUserId).Child("MetersTraveled")
            .SetValueAsync(metersTraveled).ContinueWithOnMainThread(task => {
                if (task.IsCompleted)
                {
                    Debug.Log($"Đã cập nhật MetersTraveled: {metersTraveled}m");
                    // Cập nhật highScore với cùng giá trị
                    UpdateHighScore(metersTraveled);
                }
                else if (task.IsFaulted)
                {
